...
type DiscoveryService struct {
	...
	cacheStats   map[string]*discoveryCacheStatEntry
}
...
func NewDiscoveryService(o DiscoveryServiceOptions) (*DiscoveryService, error) {
	out := &DiscoveryService{
		...
		cacheStats:   make(map[string]*discoveryCacheStatEntry),
...
func (ds *DiscoveryService) Register(container *restful.Container) {
	...
	ws.Route(ws.
		...
		To(ds.GetCacheStats).
		Writes(map[string]*discoveryCacheStatEntry{}))
...
func (ds *DiscoveryService) ListEndpoints(request *restful.Request, response *restful.Response) {
	serviceKey := request.PathParameter(ServiceKey)
...
func (ds *DiscoveryService) ListClusters(request *restful.Request, response *restful.Response) {
	...
	out, cached := ds.cache[key]
	if ds.disableCache || !cached {
		serviceCluster := request.PathParameter(ServiceCluster)
		if serviceCluster != ds.mesh.IstioServiceCluster {
			errorResponse(response, http.StatusNotFound,
				fmt.Sprintf("Unexpected %s %q", ServiceCluster, serviceCluster))
			return
		}
